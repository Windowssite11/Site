...
@app.route('/api/submit-window-request', methods=['POST'])
def submit_window_request():
    ...
    try:
        ...
        app.logger.info(f"Window request submitted: {data.get('name')}, {data.get('phone')}")
        ...
        # Логирование ошибки
        app.logger.error(f"Error submitting window request: {str(e)}")
        return jsonify({
            'success': False, 
